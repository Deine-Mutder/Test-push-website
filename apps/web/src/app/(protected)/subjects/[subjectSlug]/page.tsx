...
export default function SubjectPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setIsLoading(true);
      const s = await api.get<Subject>(`/subjects/${params.subjectSlug}`);
      if (cancelled) return;
      setSubject(s);

      if (s.placementCompleted) {
        try {
          const t = await api.get<Topic[]>(`/subjects/${params.subjectSlug}/topics`);
  ...
  async function startPlacementTest() {
    ...
    try {
      const attempt = await api.post<{ id: string }>(
        `/subjects/${params.subjectSlug}/placement-test/start`,
      );
      router.push(`/subjects/${params.subjectSlug}/placement-test?attemptId=${attempt.id}`);
    } finally {
  ...
  // -------------------------------------------------------------------
  // Zustand 1: Einstufungstest noch nicht absolviert -> Wiki gesperrt
  // -------------------------------------------------------------------
  if (!subject.placementCompleted) {
    return (
      <div className="max-w-xl mx-auto text-center py-12 animate-fade-up">
        ...
        <p className="text-muted mt-3 leading-relaxed">
          ...
          deinen aktuellen Wissensstand. So können wir dir gezielt zeigen, wo deine Stärken und
          Defizite liegen.
        </p>
  ...
  // -------------------------------------------------------------------
  // Zustand 2: Wiki freigeschaltet -> Themenliste mit Defizit-Priorisierung
  // -------------------------------------------------------------------
  return (
    <div className="animate-fade-up">
      ...
      <p className="text-muted mb-8">
        Themen mit Defizit stehen ganz oben. Wähle ein Thema, um zu lernen oder den Themen-Test
        zu starten.
      </p>
      ...
      <div className="grid gap-3">
        {(topics ?? []).map((topic) => {
          ...
          return (
            <Link
              key={topic.id}
              href={`/subjects/${params.subjectSlug}/topics/${topic.id}/wiki`}
